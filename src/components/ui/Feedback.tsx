...
export function EmptyState({ title, message }: { title: string; message?: string }) {
  return (
    <div className="rounded-lg border border-dashed border-ink-100 bg-white p-8 text-center">
      <p className="font-semibold text-ink-900">{title}</p>
      {message ? <p className="mt-2 text-sm text-ink-500">{message}</p> : null}
    </div>
  );
}
...
function ToastRow({ toast }: { toast: Toast }) {
  ...
  useEffect(() => {
    ...
    setIsPaused(false);
    timerRef.current = window.setTimeout(() => removeToast(toast.id), durationMs);
    return () => {
  ...
  function resumeDismissTimer() {
    ...
    startedAtRef.current = Date.now();
    timerRef.current = window.setTimeout(() => removeToast(toast.id), remainingMsRef.current);
    setIsPaused(false);
  }
    ...
    >
      <div className="flex items-start gap-3 px-4 pb-2 pt-3">
        ...
        <div className="min-w-0 flex-1">
          ...
          {toast.message ? <p className="mt-1 text-sm leading-snug text-ink-600">{toast.message}</p> : null}
        </div>
        <button
          type="button"
